import { shortcutsAvailable, shortcutsEnabled, setShortcutsEnabled } from '../../../shortcuts/native';
...
import {
  Alert, Linking, Pressable, Button as RNButton, Image, Platform, StyleSheet, Text, View,
} from 'react-native';
...
export function MemberSettingsScreen({ navigation }: NativeStackScreenProps<RootStackParamList, 'Settings'>) {
  ...
  const [allowShortcuts, setAllowShortcuts] = useState(shortcutsEnabled);
  ...
  if (Platform.OS === 'ios') return <SwiftUIHost modifiers={[tint(theme.blue)]} style={styles.flex}><SwiftUIForm><SwiftUISection><SwiftUIButton modifiers={[buttonStyle('plain'), foregroundStyle('primary')]} onPress={() => navigation.navigate('Account')}><SwiftUIHStack spacing={12} modifiers={[contentShape(shapes.rectangle())]}><SwiftUIRNHostView matchContents><ProfileAvatar size={42} /></SwiftUIRNHostView><SwiftUIVStack alignment="leading" spacing={2}><SwiftUIText modifiers={[font({ textStyle: 'headline' })]}>{session.user?.name ?? 'Pulpo Member'}</SwiftUIText><SwiftUIText modifiers={[font({ textStyle: 'footnote' }), foregroundStyle('secondary')]}>{session.user?.email ?? ''}</SwiftUIText></SwiftUIVStack><SwiftUISpacer /><SwiftUIImage systemName="chevron.right" size={11} color={theme.tertiary} /></SwiftUIHStack></SwiftUIButton></SwiftUISection><SwiftUISection title="Preferences">{settingsSections.slice(0, 2).map((section) => <NativeSettingsLink key={section.id} icon={section.icon} title={section.title} detail={section.detail} onPress={() => open(section.id)} />)}</SwiftUISection><SwiftUISection title="Apple Shortcuts">
      <NativeToggleRow title="Allow Shortcuts" detail="Use your signed-in account from Siri and Shortcuts. Your device must be unlocked." value={allowShortcuts} onChange={(value) => { setShortcutsEnabled(value); setAllowShortcuts(value); }} icon="square.stack.3d.up" />
      <SwiftUIText modifiers={[font({ textStyle: 'footnote' }), foregroundStyle('secondary')]}>{shortcutsAvailable ? 'Ask Pulpo returns a text reply. Start Chat returns a chat immediately; connect it to Open Chat or use Get Reply later. Find Chats searches saved conversations. Models can be selected in each shortcut. Expand Ask, Start Chat, or Continue Chat to enable Agent Mode when supported by your server and model.' : 'Install a current Pulpo native build to use Apple Shortcuts.'}</SwiftUIText>
      <SwiftUIText modifiers={[font({ textStyle: 'footnote' }), foregroundStyle('secondary')]}>{'Prompts use your account balance and the model’s default settings. Temporary chats stay out of saved history. Saved chat and model selections belong to their original account and server.'}</SwiftUIText>
      <SwiftUIButton label="Open Shortcuts" systemImage="arrow.up.forward.app" onPress={() => { void Linking.openURL('shortcuts://').catch(() => Alert.alert('Shortcuts unavailable', 'Install Apple Shortcuts to build workflows with Pulpo.')); }} />
    </SwiftUISection><SwiftUISection title="Data and support">{settingsSections.slice(2).map((section) => <NativeSettingsLink key={section.id} icon={section.icon} title={section.title} detail={section.detail} onPress={() => open(section.id)} />)}</SwiftUISection></SwiftUIForm></SwiftUIHost>;
