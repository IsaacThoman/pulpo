...
it('checkpoints pending edits while a profile is suspended and replays them only on resume', async () => {
  const personal = fixture(), work = fixture()
  const a = personal.client('personal')
  await a.open()
  a.sync.edit('new', { content: 'Offline Personal draft' })
  await a.sync.pause()
  expect(a.saved()?.pending.content).toBe('Offline Personal draft')
  expect(personal.writes).toHaveLength(0)
  const b = work.client('work')
  await b.open()
  expect(work.snapshot().state.content).toBe('')
  const resumed = personal.client('personal-resumed', a.saved())
  await resumed.open()
  expect(personal.snapshot().state.content).toBe('Offline Personal draft')
