import { beginDiagnostic, diagnosticContext, diagnosticFailure, updateDiagnostic, writeDiagnosticPayload, type DiagnosticContext } from './provider-diagnostics.js'
...
export function diagnosticFetch(defaults?: DiagnosticContext, baseFetch: typeof fetch = globalThis.fetch): typeof fetch {
  return async (input, init) => {
    ...
    const row = await beginDiagnostic(context as DiagnosticContext)
    ...
    const metadata: Record<string, unknown> = { method: init?.method ?? (input instanceof Request ? input.method : 'GET'), endpoint: url.origin + url.pathname, sdkRetryCount: safeDiagnosticText(headers.get('x-stainless-retry-count'), [], 16) }
    if (row.capture) {
      ...
      if (body instanceof FormData) {
        ...
        await writeDiagnosticPayload(row.id, 'requestPayload', diagnosticPayload(fields, 'reconstructed', secrets))
      } else if (typeof body === 'string') {
        await writeDiagnosticPayload(row.id, 'requestPayload', parseDiagnosticBody(body.slice(0, MAX_DIAGNOSTIC_BYTES), headers.get('content-type') ?? '', Buffer.byteLength(body) > MAX_DIAGNOSTIC_BYTES, secrets))
      }
    }
    await updateDiagnostic(row.id, metadata)
    let response: Response
    try { response = await baseFetch(input, init) }
    catch (error) { await updateDiagnostic(row.id, { ...diagnosticFailure(error), errorMessage: safeDiagnosticText((error as Error)?.message, secrets), durationMs: Date.now() - started }, 'failed'); throw error }
    ...
    if (!reader) { await updateDiagnostic(row.id, { durationMs: Date.now() - started }, response.ok ? 'completed' : 'failed'); return response }
    ...
    const finish = async (status: string, extra: Record<string, unknown> = {}) => {
      ...
      if (row.capture && textBody) await writeDiagnosticPayload(row.id, 'responsePayload', parseDiagnosticBody(text, type, total > bytes || status === 'cancelled', secrets))
      await updateDiagnostic(row.id, { durationMs: Date.now() - started, responseBytes: total, ...(firstTokenMs !== undefined ? { firstTokenMs } : {}), ...extra }, status)
    }
    // Read error bodies before returning: adapters often cancel them without consuming anything.
    if (!response.ok) {
      try {
        while (bytes < MAX_DIAGNOSTIC_BYTES) { const p = await reader.read(); if (p.done) break; observe(p.value); if (!textBody) break }
    ...
    const stream = new ReadableStream<Uint8Array>({
      async pull(controller) {
        try { const p = await reader.read(); if (p.done) { await finish('completed'); reader.releaseLock(); controller.close() } else { observe(p.value); controller.enqueue(p.value) } }
        catch (error) { await finish('failed', { failureStage: 'stream', errorMessage: safeDiagnosticText((error as Error).message, secrets) }); controller.error(error) }
      },
      async cancel(reason) { await reader.cancel(reason).catch(() => undefined); reader.releaseLock(); await finish('cancelled', { failureStage: 'cancelled' }) },
    })
