...
import type { ImageCatalog } from '@pulpo/contracts'
...
export function ImageGenerationSettings() {
  ...
  const help = model ? undefined : catalog.data?.data.length === 0 ? 'An admin must configure an image model first.'
  ...
  if (Platform.OS === 'ios') return <Section title="Image generation" footer={help ? <NativeText modifiers={[foregroundStyle('secondary')]}>{help}</NativeText> : undefined}>
  ...
  return <>
    ...
    </Card>{help && <Text style={{ color: theme.secondary, paddingHorizontal: 16 }}>{help}</Text>}
  </>
}
