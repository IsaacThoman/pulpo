...
export async function downloadSpeechPreview(request: FastifyRequest, reply: FastifyReply) {
...
export async function deleteSpeechPreview(request: FastifyRequest, reply: FastifyReply) {
  const admin = requireAdmin(request)
  const { id, voiceId } = request.params as { id: string; voiceId: string }
...
export async function registerSpeechPreviewRoutes(app: FastifyInstance) {
  app.get('/api/speech-models/:id/voices/:voiceId/preview', downloadSpeechPreview)
  app.post('/api/admin/speech-models/:id/voices/:voiceId/preview', { bodyLimit: MAX_PREVIEW_BYTES + 65536 }, uploadSpeechPreview)
  app.delete('/api/admin/speech-models/:id/voices/:voiceId/preview', deleteSpeechPreview)
}
