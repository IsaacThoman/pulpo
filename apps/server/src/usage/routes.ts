...
export async function loadUsageActivity(input: {
  ...
  const [summary, daily, contribution, topModels, aliases] = await Promise.all([
    db.select({
      ...
      subscriptionCoveredMicros: sql<number>`coalesce(sum(${usageEvents.weeklyCostMicros}), 0)::bigint`,
  ...
  return {
    summary: {
      ...
      subscriptionCoveredMicros: Number(totals?.subscriptionCoveredMicros ?? 0),
