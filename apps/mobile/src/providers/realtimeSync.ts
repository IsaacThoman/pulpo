...
export function stateInvalidationQueryKeys(
  ...
  if (scope === 'models' || scope === 'folders' || scope === 'settings' || scope === 'drafts') return [[scope, namespace]]
  return []
}
...
/** Collapse adjacent text deltas while preserving the final replay cursor. */
