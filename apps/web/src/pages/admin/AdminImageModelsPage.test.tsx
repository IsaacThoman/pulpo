...
it('loads an existing OpenAI model for editing without overwriting its settings', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Save' }))
  await waitFor(() => expect(mocks.api).toHaveBeenCalledWith('/api/admin/image-models/openai-image', { method: 'PATCH', body: expect.objectContaining(existing) }))
})
...
it('saves OpenAI token rates and requires an upfront reservation', async () => {
  mocks.api.mockImplementation(async (path: string) => ({ data: path.endsWith('/providers') ? [{ id: '11111111-1111-4111-8111-111111111111', name: 'OpenAI connection' }] : [] }))
  render(<AdminImageModelsPage />)
  await waitFor(() => expect(mocks.api).toHaveBeenCalledWith('/api/admin/providers'))
  fireEvent.click(screen.getByRole('button', { name: 'Add image model' }))
  fireEvent.change(screen.getByLabelText('Image provider API'), { target: { value: 'openai-images' } })
  fireEvent.change(screen.getByLabelText('ID'), { target: { value: 'token-image' } })
  fireEvent.click(screen.getByLabelText('Bill users for images'))
  fireEvent.change(screen.getByLabelText('Image billing unit'), { target: { value: 'tokens' } })
  expect((screen.getByLabelText('Image output tokens') as HTMLInputElement).value).toBe('30')
  expect(screen.queryByLabelText('USD per generated image')).toBeNull()
  fireEvent.click(screen.getByRole('button', { name: 'Save' }))
  expect(await screen.findByRole('alert')).toHaveProperty('textContent', expect.stringContaining('positive upfront reservation'))
  fireEvent.change(screen.getByLabelText('Upfront reservation (USD)'), { target: { value: '0.25' } })
  fireEvent.change(screen.getByLabelText('Text input tokens'), { target: { value: '6' } })
  fireEvent.click(screen.getByRole('button', { name: 'Save' }))
  await waitFor(() => expect(mocks.api).toHaveBeenCalledWith('/api/admin/image-models', { method: 'POST', body: expect.objectContaining({ billingUnit: 'tokens', reservationMicros: 250000, tokenPrices: expect.objectContaining({ input: 6000000, imageOutput: 30000000 }) }) }))
})
it('requires explicit Meta rates and prevents carrying OpenAI pricing into Azure', async () => {
  mocks.api.mockImplementation(async (path: string) => ({ data: path.endsWith('/providers') ? [{ id: '11111111-1111-4111-8111-111111111111', name: 'Connection' }] : [] }))
  render(<AdminImageModelsPage />)
  await waitFor(() => expect(mocks.api).toHaveBeenCalledWith('/api/admin/providers'))
  fireEvent.click(screen.getByRole('button', { name: 'Add image model' }))
  fireEvent.change(screen.getByLabelText('Image provider API'), { target: { value: 'meta-muse' } })
  fireEvent.change(screen.getByLabelText('ID'), { target: { value: 'meta-image' } })
  fireEvent.click(screen.getByLabelText('Bill users for images'))
  fireEvent.change(screen.getByLabelText('Image billing unit'), { target: { value: 'tokens' } })
  expect((screen.getByLabelText('Input tokens') as HTMLInputElement).value).toBe('')
  fireEvent.change(screen.getByLabelText('Upfront reservation (USD)'), { target: { value: '0.1' } })
  fireEvent.click(screen.getByRole('button', { name: 'Save' }))
  expect(await screen.findByRole('alert')).toHaveProperty('textContent', expect.stringContaining('Enter a token rate'))
  for (const label of ['Input tokens', 'Cached input tokens', 'Output tokens']) fireEvent.change(screen.getByLabelText(label), { target: { value: '0' } })
  fireEvent.click(screen.getByRole('button', { name: 'Save' }))
  await waitFor(() => expect(mocks.api).toHaveBeenCalledWith('/api/admin/image-models', expect.objectContaining({ body: expect.objectContaining({ billingUnit: 'tokens', tokenPrices: expect.objectContaining({ input: 0, cachedInput: 0, output: 0 }) }) })))
  fireEvent.click(screen.getByRole('button', { name: 'Add image model' }))
  fireEvent.click(screen.getByLabelText('Bill users for images'))
  expect(screen.getByRole('option', { name: 'Per token' }).hasAttribute('disabled')).toBe(true)
})
