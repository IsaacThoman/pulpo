  ...
  return <>
    <div className="flex min-w-0 flex-col items-start gap-2 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
      <div className="min-w-0 flex-1">
        <div className="text-sm font-medium">{ui("Passkeys")}</div>
        <div className="mt-0.5 text-xs text-muted-foreground">
          {data ? uit`${data.passkeys.length} of 10 added · Sign in without a password or authenticator code.` : ui("Sign in securely without a password.")}
        </div>
      </div>
      <Button variant="outline" size="sm" onClick={() => setOpen(true)}>{ui("Manage")}</Button>
    </div>

    <Dialog open={open} onOpenChange={(nextOpen) => { setOpen(nextOpen); if (!nextOpen) resetAction() }}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{ui("Passkeys")}</DialogTitle>
          <DialogDescription>{ui("Use Face ID, Touch ID, a device PIN, or a security key to sign in.")}</DialogDescription>
        </DialogHeader>

        {adding ? <div className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="passkey-name">{ui("Passkey name")}</Label>
            <Input id="passkey-name" autoFocus maxLength={80} placeholder={ui("e.g. MacBook Touch ID")} value={name} onChange={(event) => { setName(event.target.value); setError('') }} />
          </div>
          <SensitiveFields values={sensitive} requiresSecondFactor={Boolean(data?.requiresSecondFactor)} onChange={updateSensitive} />
          <p className="text-xs text-muted-foreground">{ui("Adding a passkey signs out your other devices. This device stays signed in.")}</p>
          {error && <p className="text-sm text-destructive" role="alert">{error}</p>}
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={resetAction}>{ui("Cancel")}</Button>
            <Button disabled={!name.trim() || !sensitiveReady || loading} onClick={() => void add()}>
              {loading && <Loader2 className="animate-spin" />}{ui("Add passkey")} </Button>
          </div>
        </div> : deleting ? <div className="space-y-4">
          <p className="text-sm">{ui("Delete")} <strong>{deleting.name}</strong>{ui("? You can still sign in with your password or another passkey.")}</p>
          <SensitiveFields values={sensitive} requiresSecondFactor={Boolean(data?.requiresSecondFactor)} onChange={updateSensitive} />
          <p className="text-xs text-muted-foreground">{ui("Deleting a passkey signs out your other devices. This device stays signed in.")}</p>
          {error && <p className="text-sm text-destructive" role="alert">{error}</p>}
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={resetAction}>{ui("Cancel")}</Button>
            <Button variant="destructive" disabled={!sensitiveReady || loading} onClick={() => void remove()}>
              {loading && <Loader2 className="animate-spin" />}{ui("Delete passkey")} </Button>
          </div>
          ...
          {!data ? <div className="flex justify-center py-8"><Loader2 className="animate-spin text-muted-foreground" /></div> : data.passkeys.length === 0 ? (
            <div className="rounded-lg border border-dashed px-4 py-8 text-center">
              ...
              <p className="text-sm font-medium">{ui("No passkeys yet")}</p>
              <p className="mt-1 text-xs text-muted-foreground">{ui("Add one to make sign-in faster and phishing-resistant.")}</p>
            </div>
            ...
            {data.passkeys.map((passkey) => <div key={passkey.id} className="py-3">
              {editingId === passkey.id ? <div className="flex items-center gap-2">
                ...
                <Button size="sm" disabled={!name.trim() || loading} onClick={() => void rename(passkey)}>{ui("Save")}</Button>
                <Button size="sm" variant="ghost" onClick={resetAction}>{ui("Cancel")}</Button>
              </div> : <div className="flex items-center gap-3">
                <KeyRound className="size-4 shrink-0 text-muted-foreground" />
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm font-medium">{passkey.name}</div>
                  <div className="text-xs text-muted-foreground">{ui("Added")} {formatPasskeyDate(passkey.createdAt)} · {passkey.lastUsedAt ? uit`Last used ${formatPasskeyDate(passkey.lastUsedAt)}` : ui("Never used")}</div>
                </div>
                <Button size="icon-sm" variant="ghost" aria-label={uit`Rename ${passkey.name}`} onClick={() => { resetAction(); setEditingId(passkey.id); setName(passkey.name) }}><Pencil /></Button>
                <Button size="icon-sm" variant="ghost" aria-label={uit`Delete ${passkey.name}`} onClick={() => { resetAction(); setDeleting(passkey) }}><Trash2 /></Button>
              </div>}
            </div>)}
          </div>}
          <Separator />
          <Button className="w-full" disabled={!data || data.passkeys.length >= 10} onClick={() => { resetAction(); setAdding(true) }}><Plus />{ui("Add passkey")}</Button>
        </div>}
      </DialogContent>
    </Dialog>
  </>
...
function SensitiveFields({
  ...
  return <>
    <div className="space-y-2">
      <Label htmlFor="passkey-current-password">{ui("Current password")}</Label>
    ...
    {requiresSecondFactor && <div className="space-y-2">
      <Label htmlFor="passkey-verification-code">{ui("Authenticator or recovery code")}</Label>
