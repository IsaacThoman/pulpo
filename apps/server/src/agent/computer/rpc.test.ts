...
describe('ComputerRpcClient', () => {
  it('publishes a request and resolves with the matching reply', async () => {
    ...
    const pending = client.request('c1', { kind: 'operation.status', id: 'op-1' })
    await vi.waitFor(() => expect(fake.published).toHaveLength(1))
    expect(fake.published[0]).toMatchObject({ computerId: 'c1', request: { kind: 'operation.status', id: 'op-1' } })
  ...
  it('surfaces desktop-side refusals with their code', async () => {
    const fake = fakeRedis(new Set(['c1']))
    const client = new ComputerRpcClient(fake.publisher, fake.subscriber)
    const pending = client.request('c1', { kind: 'operation.start', id: 'op-2', type: 'bash', args: { command: 'ls' } })
    await vi.waitFor(() => expect(fake.published).toHaveLength(1))
  ...
  it('reports offline when nothing answers and no presence key exists, unreachable otherwise', async () => {
    ...
    await expect(offlineClient.request('gone', { kind: 'operation.status', id: 'x' }, { timeoutMs: 30 }))
    ...
    await expect(silentClient.request('quiet', { kind: 'operation.status', id: 'x' }, { timeoutMs: 30 }))
  ...
  it('aborts a pending request when the signal fires', async () => {
    ...
    const pending = client.request('c1', { kind: 'operation.status', id: 'op-3' }, { signal: controller.signal, timeoutMs: 5_000 })
    await vi.waitFor(() => expect(fake.published).toHaveLength(1))
