...
describe.skipIf(!enabled)('image generation persistence and authorization', () => {
  ...
  it('writes through the real manager and recovers only from the original authorized lease', async () => {
    ...
    const responseId = randomUUID(), workspaceScopeId = randomUUID()
    await db.insert(responses).values({ id: responseId, chatId: isolatedChatId, userId, modelId: chatModelId, input: [], workspaceScopeId })
    await db.insert(workspaceLeases).values({ id: leaseId, workspaceScopeId, responseId, chatId: isolatedChatId, userId, controllerLeaseId: controllerId, status: 'ready', imageDigest: 'test-image' })
    try {
      ...
      await expect(new WorkspaceManager(responseId, chatId, userId).readGeneratedFile(path, controllerId)).rejects.toThrow()
      await expect(new WorkspaceManager(responseId, isolatedChatId, randomUUID()).readGeneratedFile(path, controllerId)).rejects.toThrow()
