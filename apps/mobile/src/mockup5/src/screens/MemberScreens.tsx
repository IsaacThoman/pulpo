...
const settingsSections: { id: SettingsDestination; title: string; detail: string; icon: string }[] = [
  ...
  { id: 'personalization', title: 'Personalization', detail: 'Speech and voice preferences', icon: 'sparkles' },
...
export function MemberSettingsScreen({ navigation }: NativeStackScreenProps<RootStackParamList, 'Settings'>) {
  ...
  return <Screen><PageHeader title="Settings" subtitle={new URL(instance.url).hostname} onBack={() => navigation.goBack()} />
    ...
    <SectionTitle>Preferences</SectionTitle><Card>{settingsSections.slice(0, 4).map((section, index, list) => <ListRow key={section.id} icon={section.icon} title={section.title} detail={section.detail} last={index === list.length - 1} onPress={() => open(section.id)} />)}</Card>
...
const settingTitles: Record<SettingsSection, string> = { general: 'General', interface: 'Interface', personalization: 'Personalization', data: 'Data Controls' };
  ...
  if (Platform.OS === 'ios') return <SwiftUIHost key={section === 'data' ? storageLabel : section} modifiers={[tint(theme.blue)]} style={styles.flex}><SwiftUIForm>
    ...
    {section === 'interface' && <>
      <SwiftUISection title="Conversation">
        ...
        <NativeToggleRow icon="text.bubble" title="Collapse intermediate messages" detail="Group intermediate assistant messages and activity into one expandable work section, leaving the final answer visible." value={preferences.collapseIntermediateMessages} onChange={(value) => setPreference('collapseIntermediateMessages', value)} />
    ...
    {section === 'personalization' && <><SpeechSettings /><ImageGenerationSettings /></>}
    {section === 'data' && <>
