        ...
        <DialogContent className="sm:max-w-md">
          {topUpStep === 'amount' ? <>
            ...
            <div className="space-y-4 py-2">
              ...
              <p className="text-xs text-muted-foreground">Estimate includes a 5% + $0.50 platform fee. Sales tax is calculated at checkout.</p>
            </div>
            ...
            <div className="space-y-4 py-2"><Quote credits={purchaseAmount} fee={feeCoverageAmount} charge={chargeAmount} /><div className="flex items-start gap-2 text-xs text-muted-foreground"><ShieldCheck className="mt-0.5 size-3.5 shrink-0" />Sales tax is calculated at checkout.</div>{topUpError && <p className="text-sm text-destructive">{topUpError}</p>}</div>
...
function Quote({ credits, fee, charge }: { credits: number; fee: number; charge: number }) {
  return <div className="space-y-2 rounded-lg bg-muted/50 p-4 text-sm"><div className="flex justify-between gap-4"><span className="text-muted-foreground">Credits added</span><span className="tabular-nums">{formatBalance(credits)}</span></div><div className="flex justify-between gap-4"><span className="text-muted-foreground">Platform fee</span><span className="tabular-nums">{formatBalance(fee)}</span></div><Separator className="my-2" /><div className="flex justify-between gap-4 font-medium"><span>Total before tax</span><span className="tabular-nums">{formatBalance(charge)}</span></div></div>
}
