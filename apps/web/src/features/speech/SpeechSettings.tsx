...
import { speechCatalog, speechPlayback, previewSpeech } from './playback'
...
export function SpeechSettings() {
  ...
  return <div className="space-y-5">
    ...
    <div className="space-y-2">
      <Button type="button" variant="outline" disabled={!canPreview && !previewing} aria-pressed={previewing} onClick={() => { if (previewing) speechPlayback.stop(); else void previewSpeech() }}>
        {previewing ? playback.phase === 'loading' ? <Loader2 className="size-4 animate-spin" /> : <Square className="size-4" /> : <Play className="size-4" />}
        {ui(previewing ? 'Stop preview' : 'Preview speech')}
      </Button>
      {previewing && playback.phase === 'loading' && <p role="status" className="text-sm text-muted-foreground">{ui('Generating speech…')}</p>}
