import { currentProfile, profileEq, profileInArray } from '../profiles/context.js'
...
async function readShelf(tx: Transaction, userId: string): Promise<ShelfSnapshot> {
  ...
  const rows = await tx.select().from(shelvedDrafts).where(and(profileEq(shelvedDrafts.userId, userId), isNull(shelvedDrafts.deletedAt)))
...
export async function mutateShelf(userId: string, input: ShelfMutation): Promise<ShelfSnapshot> {
  const result = await db.transaction(async (tx) => {
    ...
    const [receipt] = await tx.select().from(shelfOperations).where(and(profileEq(shelfOperations.userId, userId), profileEq(shelfOperations.operationId, input.operationId)))
    ...
    const referencedIds = [...new Set([
      ...(draft ? [draft.id] : []), ...('id' in action ? [action.id] : []), ...('targetId' in action ? [action.targetId] : []),
    ])]
    if (referencedIds.length) {
      const references = await tx.select({ userId: shelvedDrafts.userId, profileId: shelvedDrafts.profileId }).from(shelvedDrafts).where(inArray(shelvedDrafts.id, referencedIds))
      if (references.some((row) => row.userId !== userId || (currentProfile() && row.profileId !== currentProfile()!.profileId))) throw forbidden()
    }
    if (draft) {
      ...
      const [existing] = await tx.select().from(shelvedDrafts).where(profileEq(shelvedDrafts.id, draft.id))
      ...
      if (!existing) {
        ...
        const owned = attachmentIds.length ? await tx.select({ attachment: attachments }).from(attachments)
          .leftJoin(chats, profileEq(chats.id, attachments.chatId)).where(and(
            profileInArray(attachments.id, attachmentIds), profileEq(attachments.userId, userId), profileEq(attachments.status, 'ready'),
        ...
        if (attachmentIds.length) await tx.update(attachments).set({ chatId: null, shelvedAt: new Date(), updatedAt: new Date() })
          .where(profileInArray(attachments.id, attachmentIds))
    ...
    if (action.type === 'restore' || action.type === 'delete') {
      ...
      await tx.update(shelvedDrafts).set({ deletedAt: new Date(), content: '', attachmentData: [], updatedAt: new Date() })
        .where(and(profileEq(shelvedDrafts.id, action.id), profileEq(shelvedDrafts.userId, userId)))
      ...
      await tx.delete(shelvedDraftAttachments).where(and(eq(shelvedDraftAttachments.draftId, action.id),
        sql`exists (select 1 from ${shelvedDrafts} where ${and(eq(shelvedDrafts.id, action.id), profileEq(shelvedDrafts.userId, userId))})`))
    ...
    for (const [position, id] of ids.entries()) await tx.update(shelvedDrafts).set({ position, revision: user!.revision }).where(profileEq(shelvedDrafts.id, id))
...
export async function shelfAttachmentIsLive(userId: string, attachmentId: string, database: Pick<typeof db, 'select'> = db): Promise<boolean> {
  const [row] = await database.select({ id: shelvedDrafts.id }).from(shelvedDraftAttachments)
    .innerJoin(shelvedDrafts, profileEq(shelvedDrafts.id, shelvedDraftAttachments.draftId))
    .where(and(profileEq(shelvedDrafts.userId, userId), eq(shelvedDraftAttachments.attachmentId, attachmentId), isNull(shelvedDrafts.deletedAt))).limit(1)
  return Boolean(row)
}
