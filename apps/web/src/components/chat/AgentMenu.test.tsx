...
it.each([true, false])('opens without changing mode and explicitly selects the other option (enabled: %s)', async (initial) => {
  ...
  const trigger = screen.getByRole('button', { name: `Agent options, ${initial ? 'Pulpo Small' : 'Disabled'}` })
  expect(trigger.querySelector(initial ? '.lucide-bot' : '.lucide-bot-off')).not.toBeNull()
  fireEvent.keyDown(trigger, { key: 'Enter' })
  const selected = await screen.findByRole('menuitemradio', { name: initial ? 'Pulpo Small' : 'Disabled', checked: true })
  expect(selected.querySelector('.lucide-check')).not.toBeNull()
  expect(onSelect).not.toHaveBeenCalled()
  fireEvent.click(screen.getByRole('menuitemradio', { name: initial ? 'Disabled' : 'Pulpo Small', checked: false }))
  expect(onSelect).toHaveBeenCalledExactlyOnceWith(!initial)
  await waitFor(() => expect(screen.queryByRole('menu')).toBeNull())
  expect(screen.getByRole('button', { name: `Agent options, ${initial ? 'Disabled' : 'Pulpo Small'}` }).querySelector(initial ? '.lucide-bot-off' : '.lucide-bot')).not.toBeNull()
})
...
it('closes when reselecting the current option without changing mode', async () => {
  ...
  fireEvent.click(await screen.findByRole('menuitemradio', { name: 'Pulpo Small' }))
  await waitFor(() => expect(screen.queryByRole('menu')).toBeNull())
  expect(onSelect).not.toHaveBeenCalled()
})
...
it('supports keyboard navigation and Escape without changing mode', async () => {
  ...
  const first = await screen.findByRole('menuitemradio', { name: 'Pulpo Small' })
