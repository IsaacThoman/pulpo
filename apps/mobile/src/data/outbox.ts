...
import { cacheChats, cachedChat, completeOutbox, failOutbox, pendingOutbox } from './database'
import { withoutCachedChatDetails } from './cache'
...
async function settleQueuedSubmission(namespace: string, id: string, path: string, saved: MobileQueuedMessage | null, error?: string, client?: QueryClient): Promise<void> {
  ...
  const persisted = await cachedChat(namespace, chatId)
  ...
  if (!next.temporary) await cacheChats(namespace, [withoutCachedChatDetails(next)])
