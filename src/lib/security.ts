...
async function getSecretKey(): Promise<CryptoKey> {
  const digest = await crypto.subtle.digest('SHA-256', encoder.encode(config.appSecret));
...
export async function generateProxyKeySecret(): Promise<{
  ...
  const bytes = crypto.getRandomValues(new Uint8Array(24));
  const suffix = encodeBase64(bytes).replaceAll('=', '').replaceAll('/', '').replaceAll('+', '');
...
export async function cacheKeyForParts(parts: string[]): Promise<string> {
...
export function buildSessionCookie(value: string, maxAgeSeconds: number): string {
  return `${config.sessionCookieName}=${encodeURIComponent(value)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${maxAgeSeconds}`;
}
