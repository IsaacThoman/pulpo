...
export async function accessComposer(userId: string, draftId: string, write?: ComposerWrite, database: Pick<typeof db, 'transaction'> = db): Promise<ComposerAck> {
  ...
  return database.transaction(async (tx) => {
    ...
    if (draftId !== 'new') {
      const [chat] = await tx.select().from(chats).where(and(profileEq(chats.id, draftId), profileEq(chats.userId, userId), isNull(chats.deletedAt), accessibleChatCondition())).for('share')
      if (chat?.temporary) return { ok: false, error: 'temporary_composer_disabled' }
      if (!chat) {
        if (!write) {
          const [cleared] = await tx.select().from(composerDrafts).where(and(profileEq(composerDrafts.userId, userId), profileEq(composerDrafts.draftId, draftId)))
    ...
    let [row] = await tx.select().from(composerDrafts).where(and(profileEq(composerDrafts.userId, userId), profileEq(composerDrafts.draftId, draftId))).for('update')
    if (row!.state.temporary || (row!.expiresAt && row!.expiresAt <= new Date())) {
      ;[row] = await tx.update(composerDrafts).set({ content: '', state: emptyComposerState(), revision: row!.revision + 1, clearedRevision: row!.revision + 1, mutationId: null, expiresAt: null }).where(and(profileEq(composerDrafts.userId, userId), profileEq(composerDrafts.draftId, draftId))).returning()
    }
    ...
    if (patch.attachments?.length) {
      ...
      const owned = await tx.select({ attachment: attachments }).from(attachments).leftJoin(chats, profileEq(chats.id, attachments.chatId)).where(and(
        profileInArray(attachments.id, ids), profileEq(attachments.userId, userId), profileEq(attachments.status, 'ready'),
    ...
    }).where(and(profileEq(composerDrafts.userId, userId), profileEq(composerDrafts.draftId, draftId))).returning()
...
export async function composerAttachmentIsLive(userId: string, id: string, database: Pick<typeof db, 'select'> = db): Promise<boolean> {
  const [row] = await database.select({ draftId: composerDrafts.draftId }).from(composerDrafts).where(and(
    profileEq(composerDrafts.userId, userId),
...
export async function expireComposerDrafts(): Promise<void> {
  ...
  for (const row of expired) {
    ...
    await redis.publish('pulpo:composer-changes', JSON.stringify({ userId: row.userId, profileId: row.profileId ?? currentProfile()?.profileId, snapshot: snapshot(row) }))
  }
}
