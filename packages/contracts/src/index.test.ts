...
describe('shared contracts', () => {
  ...
  it('normalizes a complete management settings document', () => {
    ...
    expect(document.account).toMatchObject({
      ...
      nickname: '', animationSpeed: 1, showPromptSuggestions: true, collapseIntermediateMessages: true, showResponseCost: false, favoriteModelIds: [], agentModes: {},
    ...
    expect(managementAccountSettingsSchema.parse({ username: 'pulpo_user', collapseIntermediateMessages: false }).collapseIntermediateMessages).toBe(false)
    expect(managementAccountSettingsSchema.safeParse({ username: 'pulpo_user', collapseIntermediateMessages: 'false' }).success).toBe(false)
