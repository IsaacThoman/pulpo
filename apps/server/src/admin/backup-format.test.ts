...
describe('full backup format', () => {
  ...
  it('restores vector tables without writing generated lexical columns', () => {
    ...
    expect(FULL_BACKUP_EXPLICIT_COLUMNS.chat_turn_embeddings).toContain('chunk_index')
  })
  ...
  it('preserves values already stored in a newer backup', () => {
    const database = {
      ...
      chat_turn_embeddings: [{ chunk_index: 3 }],
    ...
    expect(database.episodic_memory_generations[0]!.index_version).toBe(2)
