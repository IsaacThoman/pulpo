...
function RequestRow({ row, open, detail, onToggle }: { row: LogRow; open: boolean; detail: Record<string, unknown> | null; onToggle: () => void }) { const live = row.status === 'in_progress'; return <><tr><td className="px-3 py-2"><button onClick={onToggle}>{open ? <ChevronDown className="size-3.5" /> : <ChevronRight className="size-3.5" />}</button></td><td className="whitespace-nowrap px-3 py-2">{new Date(row.createdAt).toLocaleTimeString()}<div className="text-muted-foreground">{row.durationMs == null ? '—' : `${(row.durationMs/1000).toFixed(1)}s`}</div></td><td className="px-3 py-2">{row.origin.toUpperCase()}<div className="max-w-36 truncate text-muted-foreground">{row.purpose} · {row.apiKey?.name ?? row.user.name}</div></td><td className="px-3 py-2">{row.requestedModelId}<div className="text-muted-foreground">→ {row.actualModelId ?? row.currentModelId ?? '—'}</div></td><td className="px-3 py-2">{row.turnNumber == null ? `Attempt ${row.retryAttempt}` : `Turn ${row.turnNumber}`}<div className="flex gap-1">{row.retryCount > 0 && <Badge variant="outline">{row.retryCount} retry</Badge>}{row.fallbackUsed && <Badge variant="outline">fallback</Badge>}{row.stickyFallbackUsed && <Badge variant="outline">sticky</Badge>}</div></td><td className="px-3 py-2"><Badge variant={row.ocrStatus === 'failed' ? 'destructive' : 'outline'}>{row.ocrStatus}</Badge></td><td className="px-3 py-2 text-right tabular-nums">{row.inputTokens + row.outputTokens}</td><td className="px-3 py-2 text-right tabular-nums">${(row.costMicros/1_000_000).toFixed(4)}</td><td className="px-3 py-2"><Badge variant={row.status === 'failed' ? 'destructive' : live ? 'secondary' : 'outline'}>{live && <span className="mr-1 inline-block size-1.5 animate-pulse rounded-full bg-blue-500" />}{row.status}</Badge>{row.errorCategory && <div className="mt-1 text-destructive">{row.errorCategory}</div>}</td></tr>{open && <tr className="bg-muted/20"><td colSpan={9} className="p-4"><pre className="max-h-96 overflow-auto whitespace-pre-wrap rounded-md border bg-background p-3 text-[11px]">{detail ? JSON.stringify(detail, null, 2) : 'Loading…'}</pre></td></tr>}</> }
