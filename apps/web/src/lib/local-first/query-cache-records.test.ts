...
it('writes only the changed response, round-trips order, and refuses torn response manifests', () => {
  const { queryClient, snapshot } = fixture()
  queryClient.clear()
  const responses = Array.from({ length: 1000 }, (_, i) => ({ id: String(i), output: ['original'] }))
  queryClient.setQueryData(['chat', 'user', 'a'], { id: 'a', responses })
  const first = splitQueryCache(snapshot(), 'account', new Map())
  expect(first.changed.size).toBe(1001)
  queryClient.setQueryData(['chat', 'user', 'a'], { id: 'a', responses: responses.map((r, i) => i === 999 ? { ...r, output: ['updated'] } : r) })
  const expected = snapshot()
  const updated = splitQueryCache(expected, 'account', first.data)
  expect([...updated.changed.keys()]).toEqual(['account:chat-data:["chat","user","a"]:response:999'])
  expect(restoreQueryCache(updated.envelope, updated.data)).toEqual(expected)
  const torn = new Map(updated.data)
  torn.delete([...updated.changed.keys()][0]!)
  expect(restoreQueryCache(updated.envelope, torn).clientState.queries).toEqual([])
})
