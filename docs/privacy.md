...
Turning Memories off cancels outstanding indexing, deletes that user’s chat embedding rows across all model generations, stops `MEMORY.md` injection, and removes the Agent editing tool. The current document and revisions less than 24 hours old are retained. Turning episodic memory off at the instance level stops chat indexing and recall but does not disable `MEMORY.md`; the dormant chat index is retained so it can resume if an administrator re-enables it. Deleting a conversation removes its derived embedding rows with it. Full instance backups include memory documents, retained revisions, episodic-memory operational data, and chat embeddings; ordinary chat exports contain conversations and responses, not `MEMORY.md` or derived embeddings.
