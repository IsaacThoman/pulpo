...
type TabId = 'overview' | 'keys' | 'providers' | 'models' | 'ocr' | 'usage' | 'playground' | 'settings' | 'simulations';
...
export class AdminPageComponent implements OnDestroy {
  ...
  constructor() {
    ...
    this.route.params.subscribe(params => {
      ...
      if (tab && ['overview', 'keys', 'providers', 'models', 'ocr', 'usage', 'playground', 'settings', 'simulations'].includes(tab)) {
        this.activeTab = tab;
      }
    });
  ...
  handleKeyboard(event: KeyboardEvent): void {
    ...
    if (event.metaKey || event.ctrlKey) {
      ...
      if (num >= 1 && num <= 9) {
        ...
        const tabs: TabId[] = ['overview', 'keys', 'providers', 'models', 'ocr', 'usage', 'playground', 'settings', 'simulations'];
  ...
  handleClickOutside(event: MouseEvent): void {
    ...
    if (this.showUpstreamDropdown) {
      ...
      const pointerDownTarget = this.pointerDownTarget instanceof HTMLElement ? this.pointerDownTarget : null;
      const isInsideDropdown = target.closest('.upstream-dropdown-shell') !== null;
      const pointerDownInsideDropdown = pointerDownTarget?.closest('.upstream-dropdown-shell') !== null;
  ...
  async refreshAll(): Promise<void> {
    ...
    try {
      const [keys, providers, models, simModels, usage, logging, ocr, refresh] = await Promise.all([
        ...
        this.api.getUsageSummary(this.usageDays),
  ...
  async refreshUsage(): Promise<void> {
    try {
      this.usage = await this.api.getUsageSummary(this.usageDays);
    } catch (err) {
      this.showError(this.normalizeError(err));
    }
  }
  ...
  editModel(model: ProxyModel): void {
    ...
    if (model.hasProviderApiKey && model.usesCustomProvider && !this.providerKeyStorage.has(this.getModelProviderKeyId(model.id))) {
  ...
  isProviderKeyVisible(): boolean {
    return this.currentProviderKeyId ? this.visibleProviderKeys.has(this.currentProviderKeyId) : false;
  }
  ...
  async saveModel(): Promise<void> {
    try {
      ...
      const payload = { 
        ...
        firstTokenTimeoutEnabled: fallbackEnabled ? this.modelDraft.firstTokenTimeoutEnabled : false,
      ...
      if (this.editingModelId) {
        ...
        if (this.modelDraft.providerApiKey && !this.modelDraft.providerId) {
          this.providerKeyStorage.set(this.getModelProviderKeyId(this.editingModelId), this.modelDraft.providerApiKey);
        }
        ...
        if (this.modelDraft.providerApiKey && response.item.id && !this.modelDraft.providerId) {
          this.providerKeyStorage.set(this.getModelProviderKeyId(response.item.id), this.modelDraft.providerApiKey);
        }
  ...
  async handleFileInput(event: Event): Promise<void> {
    ...
    for (const file of files) {
      if (file.type.startsWith('image/')) {
        next.push({ kind: 'image', name: file.name, payload: await this.readAsDataUrl(file) });
      } else {
        next.push({ kind: 'text', name: file.name, payload: await file.text() });
      }
    }
  ...
  async sendPlaygroundPrompt(): Promise<void> {
    ...
    for (const attachment of this.attachments.filter(a => a.kind === 'image')) {
      userContent.push({ type: 'image_url', image_url: { url: attachment.payload } });
    }

    const messages: Array<Record<string, unknown>> = [];
    if (this.playgroundSystemPrompt.trim()) {
      messages.push({ role: 'system', content: this.playgroundSystemPrompt.trim() });
    }
    messages.push({
      role: 'user',
  ...
  private async sendStreaming(messages: Array<Record<string, unknown>>): Promise<void> {
    const response = await fetch('/api/admin/playground/chat', {
      ...
      credentials: 'include',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ model: this.playgroundModelId, messages, stream: true }),
    });
