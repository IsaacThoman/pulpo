...
Persistent previews that applied the shelf under its earlier 0061/0062 numbering are reconciled under the migration advisory lock. The recovery requires the exact original shelf checksum, applies skipped upstream migrations and records the current shelf revision in one transaction, and preserves existing drafts and attachment references. Applied migration files and historical records remain unchanged. The PostgreSQL CI job covers both legacy versions, fresh installation, repeat startup, and rollback after a failed recovery using a separate disposable `pulpo_migration_test` database.
