...
describe('OpenRouter cache controls on the wire', () => {
  it('sends the chat cache opt-in through the OpenAI Responses client', async () => {
    const transport = mockTransport(0, 4800)
    const client = new OpenAI({ apiKey: 'test', baseURL, fetch: transport.fetch })
    const stream = await client.responses.create({
      ...providerPromptCacheParameters(baseURL, model.id),
      model: model.id, input: 'Hello', stream: true, store: false,
    })
    for await (const event of stream) {
      expect(event.type).toBe('response.completed')
    }
    expect(transport.payloads[0]).toMatchObject({ cache_control: { type: 'ephemeral' } })
  })

  it.each([[0, 4800], [4800, 100]])('preserves agent caching and usage (%i read, %i written)', async (read, written) => {
    ...
    const result = await openAIResponsesApi().streamSimple(model, context, {
      ...
      samplingParams: agentSamplingParameters(baseURL, providerPromptCacheParameters(baseURL, model.id)),
    }).result()
