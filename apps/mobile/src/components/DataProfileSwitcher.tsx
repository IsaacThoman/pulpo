...
export function DataProfileSwitcher() {
  ...
  return <>
    <Pressable accessibilityRole="button" accessibilityLabel={`Switch profile: ${active.name}`} onPress={() => setOpen(true)} style={styles.trigger}><Text numberOfLines={1} style={[styles.name, { color: theme.text }]}>{active.name}</Text><Text style={{ color: theme.secondary }}>⌄</Text></Pressable>
...
const styles = StyleSheet.create({
  screen: { flex: 1 }, header: { padding: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, title: { fontSize: 24, fontWeight: '700' }, content: { padding: 20, paddingTop: 0, gap: 14 }, description: { fontSize: 15, lineHeight: 22, marginBottom: 8 }, trigger: { flexDirection: 'row', alignItems: 'center', gap: 10, minHeight: 44, maxWidth: 240 }, name: { fontSize: 16, fontWeight: '600', flexShrink: 1 }, colorCheck: { color: '#fff', fontSize: 13, fontWeight: '700' }, row: { flexDirection: 'row', alignItems: 'center', borderRadius: 16, padding: 12, gap: 6 }, select: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 12, minHeight: 44 }, action: { padding: 8, minHeight: 44, justifyContent: 'center' }, input: { padding: 16, borderRadius: 14, borderWidth: 1, fontSize: 17 }, colors: { flexDirection: 'row', gap: 12, paddingVertical: 10 }, color: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' }, save: { minHeight: 50, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginTop: 6 },
})
