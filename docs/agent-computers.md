...
- **Transport.** The desktop's Electron main process opens a dedicated Socket.IO namespace `/computer` using its stored session token, a persistent installation credential, and an announce payload (`computerAnnounceSchema`). The API replica that holds the socket keeps it in a local map, records presence in Redis (`pulpo:computer:<id>:online`, 60 s TTL refreshed by heartbeats), and relays worker requests published on `pulpo:computer-requests`, publishing replies on `pulpo:computer-replies`. Each request names exactly one socket owner; other replicas cannot relay it. Presence refresh and removal use atomic compare operations. Server-originated events (approval decisions, access changes, revocation) travel on `pulpo:computer-events`.
...
- **Data model.** `agent_computers`, `agent_computer_pairings`, `agent_tool_approvals`, plus `chats.workspace_computer_id`, `responses.workspace_computer_id`, and `workspace_leases.kind/computer_id` (migration `0076_agent_computers`).
- **Selection rules.** `responses/service.ts` resolves the workspace: explicit selection, else the chat's last selected computer, else the sandbox. Selecting a computer requires a signed-in device session (API keys and admin chat access are refused), the admin flag `agent.computersEnabled`, and either owning the computer or holding an approved pairing while remote access is on. Each response persists its initiating session. Authorization is checked on every response, including inherited and queued selections, and again before every relayed request. Unavailable computers fail explicitly instead of silently rerouting to the cloud.
...
- **Approvals default on** for commands and file writes. Before starting a gated operation, the desktop verifies the durable server approval against a SHA-256 digest of the chat, operation ID, tool, complete arguments, computer, working root, and access mode. Changed actions cannot reuse approvals; identical retries survive missed decision events and desktop restarts. Approvals decided from web or mobile are trusted through the server.
- **Device identity.** A random 256-bit installation credential is stored with owner-only file permissions; the server stores its hash. A new login session cannot claim a computer using its public ID. Legacy computers may enroll a credential only from the already bound owning session. Removed IDs remain revoked; explicitly re-enabling registers a fresh identity.
- **Pairing per device session.** `allow_remote` is off by default. Only the owning desktop connection can generate a code. The server stores its hash in Redis and consumes it atomically. Pairing still requires the same Pulpo account. Revoking a session, deleting the computer, or turning remote access off revokes pairings; owned computers go offline when their desktop session is revoked.
- **Kill switch.** Disabling or deleting a computer expires its leases, cancels pending approvals, kills running processes on the desktop, and disconnects the socket. Revoking a pairing also cancels that session’s responses and active desktop operations. Sign-out and socket disconnection cancel active operations and close incomplete uploads. Stale connections cannot reclaim ownership through heartbeats.
...
- **Desktop files.** `<userData>/computer.json` (config, stable computer id, private installation credential), `<userData>/agent-workspace/chats/<chat-id>/operations` (operation journal), `<userData>/agent-workspace/chats/<chat-id>/attachments` (staged attachments).
...
Past approval outcomes appear only inside expanded work details on web, desktop, and mobile. Pending decisions remain visible outside the disclosure so Approve/Deny are always accessible; settled outcomes are hidden entirely when work details are disabled.

## Security protocol upgrade

Migration `0078_computer_security` adds the installation credential hash, exact-action approval digest, and response requester session. Update both server and desktop. Old in-flight computer responses without session attribution and old approvals without a digest fail closed; send a fresh follow-up. A removed computer is recovered by explicitly turning access off and on, which creates a new identity and requires new pairings.

The regression suite `security.integration.test.ts` runs against a disposable `pulpo_computers_test` database and Redis with `PULPO_COMPUTER_SECURITY_TESTS=true`. CI runs it with real Socket.IO clients on two independent API instances. Desktop tests cover changed-action rejection, restart recovery, remote cancellation, and removal recovery.

Computer migrations 0076–0078 follow dev’s 0075 lossless-storage migration. Existing computer previews recover the renumbered migrations by their unchanged SQL checksums, preserving registrations, credentials, and approvals. The lossless-storage maintenance cutover guard still applies before recovery; follow `docs/production-deployments.md` for an existing database.
