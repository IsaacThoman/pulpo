...
const mocks = vi.hoisted(() => ({ setSession: vi.fn(), getEnabled: vi.fn(() => true), setEnabled: vi.fn(), getScope: vi.fn(() => 'scope') }))
vi.mock('expo-modules-core', () => ({ requireOptionalNativeModule: () => mocks }))
import { clearShortcutsSession, syncShortcutsSession } from './native.ios'
