...
export function EpisodicMemorySection() {
  ...
  return <div>
    <Section title={ui('Episodic memory')} hint="Generate embeddings locally and recall relevant material from a user's previous chats when their Memories setting is enabled.">
      <Toggle
        label={ui('Enable relevant-chat recall')}
        hint="Enabling downloads the selected model and backfills eligible chats for users who opted into Memories. Disabling pauses indexing and recall without deleting the active index."
      ...
      <label className="flex items-start justify-between gap-6 text-sm">
        ...
        >
          {status.profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.label} · {profile.dimension}d · ~{bytes(profile.approximateSizeBytes)}</option>)}
        </select>
      </label>
      <label className="flex items-start justify-between gap-6 text-sm">
    ...
    <Section title={ui('Local runtime')} hint={`Pulpo connects to ${selected.model} through the deployment's PULPO_OLLAMA_URL.`}>
      <div className="flex items-center gap-2 text-sm">
        ...
        <span>{status.ollama.healthy ? `Ollama ${status.ollama.version ?? ''} is healthy` : status.ollama.error ?? ui('Ollama is unavailable')}</span>
      </div>
      <div className="text-sm">{installed ? `${selected.label} installed · ${installed.digest.slice(0, 19)}… · ${bytes(installed.size)}` : `${selected.label} is not installed yet`}</div>
    </Section>

    <Section title={ui('Index status')} hint="Download and backfill run on a dedicated queue and never consume response-generation concurrency.">
      ...
      {generation && <div className="space-y-2 text-sm">
        <div className="flex justify-between"><span className="capitalize">{generation.status}</span><span>{generation.profile} · {generation.dimension} dimensions</span></div>
        ...
        {generation.status === 'indexing' && <div>{generation.completedItems} / {generation.totalItems} items · {generation.failedItems} failed</div>}
