...
test("admin flow covers setup, keys, models, OCR, usage, and playground", async ({ page }) => {
  ...
  if (
    await page.getByRole("heading", { name: "Create the admin account" })
      .isVisible()
  ) {
  ...
  await expect(page.locator("code").filter({ hasText: "llmpx_" }).first())
    .toBeVisible();
  ...
  await page.getByLabel("Provider base /v1 URL").fill(
  ...
  await page.getByLabel("OCR provider base /v1 URL").fill(
    "http://127.0.0.1:4010/v1",
  );
  ...
  await page.getByRole("button", { name: "Playground Chat test bench" })
    .click();
  ...
  await expect(page.getByText("Echoed from upstream:", { exact: false }))
    .toBeVisible();
  await expect(page.getByText("MOCK OCR OUTPUT", { exact: false }))
    .toBeVisible();

  await page.getByRole("button", { name: "Usage Logs and costs" }).click();
  await page.getByRole("button", { name: "Refresh" }).click();
  await expect(page.getByText("Daily usage")).toBeVisible();
  await expect(page.getByRole("cell", { name: "playground" }).first())
    .toBeVisible();
});
