...
import { OPENAI_SPEECH_PRESET, SPEECH_DEFAULT_PREVIEW_TEXT, SPEECH_MAX_PREVIEW_TEXT_LENGTH, type SpeechModel } from '@pulpo/contracts'
...
export function SpeechVoiceEditor({ value, onChange, renderAudioSettings, mistral = false }: { value: VoiceSettings; onChange: (value: VoiceSettings) => void; renderAudioSettings?: (index: number) => ReactNode; mistral?: boolean }) {
  const id = useId()
  ...
  return <fieldset className="min-w-0 space-y-3">
    ...
    <div className="space-y-2">
      ...
      {voices.map((voice, index) => <div key={index} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_3rem_2.25rem] items-start gap-2">
        ...
        <label className="col-span-4 block space-y-1 text-sm">
          <span>{ui('Preview text')}</span>
          <Textarea aria-label={ui('Preview text for {{name}}', { name: voice.label || voice.id || String(index + 1) })} value={voice.previewText ?? ''} maxLength={SPEECH_MAX_PREVIEW_TEXT_LENGTH} placeholder={SPEECH_DEFAULT_PREVIEW_TEXT} rows={2} onChange={event => update(index, { previewText: event.target.value || null })} />
          <span className="block text-xs text-muted-foreground">{ui('Leave blank to use the default preview text.')}</span>
        </label>
        {renderAudioSettings && <div className="col-span-4 pb-2">{renderAudioSettings(index)}</div>}
      </div>)}
    </div>
