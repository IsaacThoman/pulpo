...
import { isValidAuthenticatorCode, isValidSecondFactorCode, normalizeAuthenticatorCode, normalizeSecondFactorCode } from '../../../auth/twoFactorInput';
...
export function PasskeysScreen({ navigation }: NativeStackScreenProps<RootStackParamList, 'Passkeys'>) {
  ...
  const sensitiveReady = Boolean(currentPassword && (!data?.requiresSecondFactor || isValidSecondFactorCode(verificationCode)));
...
export function TwoFactorScreen({ navigation }: NativeStackScreenProps<RootStackParamList, 'TwoFactor'>) {
  ...
  return <Screen><PageHeader title="Two-Factor Authentication" onBack={() => action === 'idle' ? navigation.goBack() : reset()} />
    ...
    {(action === 'setup' || action === 'regenerate' || action === 'disable') && <><SectionTitle>Confirm security change</SectionTitle><Field label="Current password" secureTextEntry autoComplete="current-password" value={currentPassword} onChangeText={setCurrentPassword} />{status?.enabled ? <Field label="Authenticator or recovery code" autoCapitalize="characters" autoComplete="one-time-code" autoCorrect={false} spellCheck={false} value={verificationCode} onChangeText={(value) => { setVerificationCode(normalizeSecondFactorCode(value)); if (error) setError(''); }} /> : null}{error ? <Text accessibilityRole="alert" style={[styles.twoFactorError, { color: theme.red }]}>{error}</Text> : null}<PrimaryButton label={action === 'disable' ? 'Disable two-factor authentication' : action === 'regenerate' ? 'Generate recovery codes' : 'Continue'} variant={action === 'disable' ? 'destructive' : 'primary'} loading={loading} disabled={!currentPassword || (Boolean(status?.enabled) && !isValidSecondFactorCode(verificationCode))} onPress={action === 'setup' ? begin : change} /></>}
    {action === 'enroll' && enrollment && <><Text style={[styles.twoFactorHelp, { color: theme.secondary }]}>Scan this QR code in your authenticator app, or copy the manual key.</Text><Image source={{ uri: enrollment.qrCodeDataUrl }} accessibilityLabel="Authenticator enrollment QR code" style={styles.twoFactorQr} /><Card style={styles.twoFactorKey}><Text selectable style={[styles.twoFactorKeyText, { color: theme.text }]}>{enrollment.manualKey}</Text><PrimaryButton label="Copy manual key" variant="secondary" icon="doc.on.doc" onPress={() => { void Clipboard.setStringAsync(enrollment.manualKey); }} /></Card><Field label="Six-digit authenticator code" autoComplete="one-time-code" autoCorrect={false} keyboardType="number-pad" spellCheck={false} value={confirmationCode} onChangeText={(value) => { setConfirmationCode(normalizeAuthenticatorCode(value)); if (error) setError(''); }} />{error ? <Text accessibilityRole="alert" style={[styles.twoFactorError, { color: theme.red }]}>{error}</Text> : null}<PrimaryButton label="Confirm and enable" loading={loading} disabled={!isValidAuthenticatorCode(confirmationCode)} onPress={confirm} /></>}
