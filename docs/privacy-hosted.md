...
Turning Memories off cancels outstanding indexing, deletes your chat embedding rows across all model generations, stops `MEMORY.md` injection, and removes the Agent editing tool. Your current document and revisions less than 24 hours old are retained. If Pulpo disables episodic memory service-wide, chat indexing and recall stop while `MEMORY.md` remains available and the dormant chat index is retained. Deleting a conversation removes its derived embedding rows with it. Full hosted-service backups include memory documents, retained revisions, episodic-memory operational data, and chat embeddings; ordinary chat exports contain conversations and responses, not `MEMORY.md` or derived embeddings.
