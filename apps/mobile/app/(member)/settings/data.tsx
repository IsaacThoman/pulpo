...
export default function DataSettingsScreen() {
  ...
  const { data } = useQuery({ queryKey: ['attachment-usage', namespace], queryFn: () => apiRequest<{ usedBytes: number; reservedBytes: number; limitBytes: number }>('/api/attachments/usage') })
  ...
  return <Screen><View style={{ marginHorizontal: -18 }}><SettingsHeader title="Data and storage" /></View>
    <SectionTitle>File storage</SectionTitle><Card><ListRow title="Storage used" detail="Uploaded and model-created files." value={data ? `${bytes(data.usedBytes + data.reservedBytes)} of ${bytes(data.limitBytes)}` : 'Loading…'} last /></Card>
