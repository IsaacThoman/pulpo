...
describe('public chat DTOs', () => {
  ...
  it('does not expose internal response persistence fields', () => {
    const row = {
      ...
      executionMode: 'stream' as const, agentMode: true, workspace: null, workspaceGeneration: 0, workspaceWait: null, agentCapacityAction: null,
  ...
  it('sends output once in compact history while retaining the legacy shape', () => {
    const row = {
      ...
      executionMode: 'stream' as const, agentMode: false, workspace: null, workspaceGeneration: 0, workspaceWait: null, agentCapacityAction: null,
  ...
  it('bounds a three-edit bundle that repeats the same multi-megabyte image context', () => {
    ...
    const rows = [0, 1, 2].map((index) => ({
      ...
      workspace: null, workspaceGeneration: 0, workspaceWait: null, agentCapacityAction: null, input: [{ role: 'user', content: 'edit' }], instructions: null,
  ...
  it('returns the activated lineage body with inactive branches left as stubs', () => {
    const rows = [0, 1].map((index) => ({
      ...
      workspace: null, workspaceGeneration: 0, workspaceWait: null, agentCapacityAction: null, input: [{ role: 'user', content: `prompt ${index}` }],
