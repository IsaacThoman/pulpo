...
export function MaterialMenu({ label, icon, actions = EMPTY_ACTIONS, sections = EMPTY_SECTIONS, text, compact, image, centered, disabled = false }: MenuProps) {
  ...
  return <Host style={{ width: triggerWidth, maxWidth: '100%', height: 48, flexShrink: text && compact ? 1 : 0 }} ignoreSafeAreaKeyboardInsets><DropdownMenu expanded={expanded} onDismissRequest={dismiss} modifiers={text ? [wrapContentWidth(centered ? 'centerHorizontally' : 'start')] : undefined}>
    ...
    </TextButton> : <IconButton enabled={!disabled} colors={{ contentColor: colors.onSurface }} onClick={() => setExpanded(true)}><Icon source={materialIcon(icon)} size={24} contentDescription={label} /></IconButton>}</DropdownMenu.Trigger>
