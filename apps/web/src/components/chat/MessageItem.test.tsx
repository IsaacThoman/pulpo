    ...
    const markup = renderToStaticMarkup(<MessageItem chat={chat} activeModelId="model-1" streaming
      message={assistant({
        ...
        outputItems: [
          ...
          { type: 'pulpo_tool', status: 'completed', tool: 'web_search', durationMs: 1_000 },
          { type: 'message', content: [{ type: 'output_text', text: 'First reply' }] },
          { type: 'pulpo_tool', status: 'completed', tool: 'web_search', durationMs: 4_000 },
...
describe('computer approvals', () => {
  ...
  it.each([true, false])('submits the chat decision from the visible card (approve: %s)', async (approved) => {
    ...
    const { MessageItem } = await import('./MessageItem')
    const view = render(<MessageItem chat={chat} message={assistant({ done: false, outputItems: [tool, approval] })} streaming activeModelId="model-1" />)
    fireEvent.click(view.getByRole('button', { name: approved ? 'Approve' : 'Deny' }))
