    ...
    const execute = async () => {
      if (forceSafari || !canUseNativePasskeys(instanceUrl)) {
        ...
        try {
          ...
          setForceSafari(true);
          setVerificationCode('');
          setError(`Native passkeys are not configured for this domain. ${data?.requiresSecondFactor ? 'Enter a fresh authenticator or recovery code, then continue securely in your browser.' : 'Continue securely in your browser.'}`);
          return;
        }
...
const settingsSections: { id: SettingsDestination; title: string; detail: string; icon: string }[] = [
  ...
  { id: 'agent', title: 'Agent', detail: 'Image generation and agent tools', icon: 'sparkles' },
...
export function MemberSettingsScreen({ navigation }: NativeStackScreenProps<RootStackParamList, 'Settings'>) {
  ...
  return <Screen><PageHeader title="Settings" subtitle={new URL(instance.url).hostname} onBack={() => navigation.goBack()} />
    ...
    <SectionTitle>Preferences</SectionTitle><Card>{settingsSections.slice(0, 4).map((section, index) => <ListRow key={section.id} icon={section.icon} title={section.title} detail={section.detail} last={index === 3} onPress={() => open(section.id)} />)}</Card>
    <SectionTitle>Data and support</SectionTitle><Card>{settingsSections.slice(4).map((section, index, list) => <ListRow key={section.id} icon={section.icon} title={section.title} detail={section.detail} last={index === list.length - 1} onPress={() => open(section.id)} />)}</Card>
  </Screen>;
}
...
const settingTitles: Record<SettingsSection, string> = { agent: 'Agent', imageGeneration: 'Image generation', speech: 'Speech', general: 'General', interface: 'Interface', data: 'Data Controls' };
...
export function SettingsDetailScreen({ navigation, route }: NativeStackScreenProps<RootStackParamList, 'SettingsDetail'>) {
  ...
  if (section === 'imageGeneration') return <ImageGenerationSettings onBack={() => navigation.goBack()} />;
  ...
  if (Platform.OS === 'ios') return <SwiftUIHost key={section === 'data' ? storageLabel : section} modifiers={[tint(theme.blue)]} style={styles.flex}><SwiftUIForm>
    ...
    {section === 'interface' && <>
      <SwiftUISection title="Conversation">
        <NativeToggleRow icon="text.bubble" title="Show prompt suggestions" detail="Show suggested prompts in new chats." value={preferences.showPromptSuggestions} onChange={(value) => setPreference('showPromptSuggestions', value)} />
      ...
      <SwiftUISection title="Speech"><SwiftUIButton label="Speech" systemImage="speaker.wave.2" onPress={() => navigation.push('SettingsDetail', { section: 'speech' })} /></SwiftUISection>
    ...
    {section === 'agent' && (
      <SwiftUISection title="Image generation"><SwiftUIButton label="Image generation" systemImage="photo" onPress={() => navigation.push('SettingsDetail', { section: 'imageGeneration' })} /></SwiftUISection>
    )}
    {section === 'data' && <>
