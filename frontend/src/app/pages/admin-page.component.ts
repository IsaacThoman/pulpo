...
export class AdminPageComponent implements OnDestroy {
  ...
  handleClickOutside(event: MouseEvent): void {
    ...
    if (this.showUpstreamDropdown) {
      ...
      const pointerDownTarget = this.pointerDownTarget instanceof HTMLElement
        ? this.pointerDownTarget
        : null;
      const isInsideDropdown =
        target.closest(".upstream-dropdown-shell") !== null;
  ...
  closeModalFromOverlay(
    ...
    if (
      ...
      this.pointerDownTarget !== event.currentTarget
    ) {
      return;
    }
  ...
  async refreshAll(): Promise<void> {
    ...
    this.showError("");
    try {
      const [keys, providers, models, simModels, usage, logging, ocr, refresh] =
        await Promise.all([
  ...
  async saveKeyEdit(): Promise<void> {
    ...
    try {
      ...
      if (this.keyEditDraft.name !== key.name) {
        await this.api.updateProxyKey(this.editingKeyId, {
          name: this.keyEditDraft.name,
        });
      }
      ...
      if (this.keyEditDraft.isActive !== key.isActive) {
        await this.api.updateProxyKey(this.editingKeyId, {
          isActive: this.keyEditDraft.isActive,
        });
      }
  ...
  trackByRequestId(_index: number, item: { id: string }): string {
    return item.id;
  }
  ...
  editModel(model: ProxyModel): void {
    ...
    this.modelDraft = {
      ...
      providerApiKey:
        this.providerKeyStorage.get(this.getModelProviderKeyId(model.id)) || "",
  ...
  getProviderKeyDisplay(): string {
    ...
    if (!storageId) return "";
    const key = this.modelDraft.providerApiKey ||
      this.providerKeyStorage.get(storageId);
    if (!key) return "";
  ...
  async copyProviderKeyForProvider(): Promise<void> {
    if (!this.editingProviderId) return;
    const key = this.providerDraft.apiKey ||
      this.providerKeyStorage.get(this.editingProviderId);
    if (!key) return;
    try {
      await navigator.clipboard.writeText(key);
      this.showNotice("Provider key copied");
  ...
  get selectedProvider(): Provider | null {
    return this.providers.find((provider) =>
      provider.id === this.modelDraft.providerId
    ) || null;
  }

  get selectedPlaygroundModel(): ProxyModel | null {
    return this.proxyModels.find((model) =>
      model.id === this.playgroundModelId
    ) || null;
  }
  ...
  onModelProviderChange(): void {
    ...
    this.modelDraft.providerBaseUrl = this.modelDraft.providerBaseUrl ||
  ...
  get selectedOcrProvider(): Provider | null {
    return this.providers.find((provider) =>
      provider.id === this.ocrDraft.providerId
    ) || null;
  }
  ...
  onOcrProviderChange(): void {
    ...
    this.ocrDraft.providerBaseUrl = this.ocrDraft.providerBaseUrl ||
      "https://api.openai.com/v1";
  }
  ...
  get filteredUpstreamModels(): string[] {
    ...
    return this.providerModelNames.filter((name) =>
  ...
  async saveModel(): Promise<void> {
    try {
      ...
      const stickyEnabled = fallbackEnabled &&
        this.modelDraft.stickyFallbackSeconds > 0;
      const payload = {
        ...this.modelDraft,
        ...
        fallbackModelId: fallbackEnabled
          ? this.modelDraft.fallbackModelId || null
          : null,
      ...
      if (this.editingModelId) {
        ...
        if (
          this.modelDraft.providerApiKey && response.item.id &&
          !this.modelDraft.providerId
        ) {
          this.providerKeyStorage.set(
  ...
  async exportMigration(): Promise<void> {
    ...
    try {
      const { blob, filename } = await this.api.exportMigration(
  ...
  async sendPlaygroundPrompt(): Promise<void> {
    ...
    for (
      const attachment of this.attachments.filter((a) => a.kind === "image")
    ) {
      userContent.push({
        type: "image_url",
        ...
        for (const line of dataLines) {
          ...
          try {
            ...
            if (typeof reasoningDelta === "string") {
              assistantMsg.reasoningText = (assistantMsg.reasoningText || "") +
                reasoningDelta;
            }
          } catch {
  ...
  editSimModel(model: SimModel): void {
    ...
    this.simModelDraft = {
      ...
      segments: model.segments.map((s) =>
        s.type === "text"
          ? { ...s, maxUpdatesPerSecond: s.maxUpdatesPerSecond ?? 10 }
          : { ...s }
      ),
    };
    this.showSimModelModal = true;
  ...
  moveSegment(index: number, direction: "up" | "down"): void {
    if (direction === "up" && index > 0) {
      ...
      this.simModelDraft.segments[index] =
        this.simModelDraft.segments[index - 1];
      this.simModelDraft.segments[index - 1] = temp;
    } else if (
      direction === "down" && index < this.simModelDraft.segments.length - 1
    ) {
      const temp = this.simModelDraft.segments[index];
      this.simModelDraft.segments[index] =
        this.simModelDraft.segments[index + 1];
