...
import { buttonStyle, font, foregroundStyle, frame, lineLimit, multilineTextAlignment, textFieldStyle, tint } from '@expo/ui/swift-ui/modifiers';
...
export function SettingsDetailScreen({ navigation, route }: NativeStackScreenProps<RootStackParamList, 'SettingsDetail'>) {
  ...
  if (Platform.OS === 'ios') return <SwiftUIHost modifiers={[tint(theme.text)]} style={styles.flex}><SwiftUIForm>
    {section === 'general' && <>
      ...
      <SwiftUISection title="Behavior">
        <NativeToggleRow icon="return" title="Send with Enter" detail="Hardware keyboard behavior." value={preferences.sendWithEnter} onChange={(value) => setPreference('sendWithEnter', value)} />
      </SwiftUISection>
    </>}
    {section === 'interface' && <>
