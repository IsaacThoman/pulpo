...
export function LoggingSection() {
  ...
  return <div>
    ...
    <div className="mb-4 flex gap-3 rounded-lg border border-amber-500/40 bg-amber-500/10 p-4 text-sm"><AlertTriangle className="mt-0.5 size-4 shrink-0 text-amber-600" /><div><div className="font-medium">{ui("Sensitive data warning")}</div><p className="mt-1 text-xs text-muted-foreground">{ui("Exact payload logging may retain prompts, outputs, reasoning, tool data, file references, and OCR image data. Turning it off immediately clears retained bodies. Changing retention reapplies the selected limit from each request's creation time.")}</p></div></div>
