...
import type { Prisma as PrismaTypes, PrismaClient, Provider, ProxyModel } from 'npm:@prisma/client';
...
export function toAdminProxyModelJson(
  model: ProxyModel & { provider?: { name: string } | null; fallbackModel?: { displayName: string } | null },
) {
  return {
    id: model.id,
...
async function assertNoFallbackCycle(
  ...
  while (currentModelId) {
    ...
    const nextModel: { fallbackModelId: string | null } | null = await prisma.proxyModel.findUnique({
...
export async function fetchProviderModels(
  ...
  return response.data.map((model) => model.id).sort((left, right) => left.localeCompare(right));
}
...
export async function persistModelInput(
  ...
  if ((input.maxRetries ?? 0) === 0 && (input.firstTokenTimeoutEnabled || input.slowStickyEnabled)) {
  ...
  if (input.slowStickyEnabled && (input.stickyFallbackSeconds ?? 0) <= 0) {
    throw new Error('Sticky block seconds must be greater than 0 to enable slow sticky fallback');
  }
...
function extractUsage(payload: Record<string, unknown> | null | undefined): UsageShape {
...
function computeCost(model: ProxyModel, usage: UsageShape): number {
  ...
  const inputCost =
    (freshInputTokens * Number(model.inputCostPerMillion)) / 1_000_000;
  const cachedCost =
    (usage.cachedInputTokens * Number(model.cachedInputCostPerMillion)) / 1_000_000;
  const outputCost =
...
async function recordUsageLog(
  ...
  const usage = input.responsePayload && typeof input.responsePayload === 'object'
...
async function attemptChatCompletion(
  ...
  try {
    const providerApiKey = await decryptSecret(input.model.providerApiKeyEncrypted);
    const shouldStream = Boolean(input.body.stream);

    const requestBody: Record<string, unknown> = {
      ...(input.model.customParams && typeof input.model.customParams === 'object'
    ...
    // Apply OCR if needed
    if (Array.isArray(requestBody.messages) && input.model.interceptImagesWithOcr) {
      requestBody.messages = await applyOcrToMessages(
        prisma,
        requestBody.messages as Array<{ role: 'system' | 'user' | 'assistant'; content: unknown }>,
      );
    }
    ...
    const upstreamResponse = await fetch(`${input.model.providerBaseUrl}/chat/completions`, {
      method: 'POST',
      headers: {
    ...
    return { response: upstreamResponse, success: true, upstreamRequestId, abortController };
  } catch (error) {
    return {
      response: new Response(
        JSON.stringify({
          error: 'Upstream fetch failed',
          details: error instanceof Error ? error.message : 'Unknown upstream fetch error',
        }),
        { status: 502, headers: { 'content-type': 'application/json' } },
      ),
      success: false,
      errorMessage: error instanceof Error ? error.message : 'Failed to reach upstream provider',
      abortController,
    };
  }
}
...
async function prepareStreamingResponse(input: {
  ...
  let rejectFirstToken: (error: Error) => void = () => undefined;
      ...
      try {
        while (true) {
          ...
          if (done) {
            ...
            if (result.sawFirstToken) {
              resolveFirstToken();
            } else {
              rejectFirstToken(new Error('Upstream stream completed before the first token'));
            }
        ...
        const streamError = error instanceof Error ? error : new Error('Failed to proxy upstream stream');
        rejectFirstToken(streamError);
  ...
  if (!input.model.firstTokenTimeoutEnabled) {
    return { ok: true, response, completion };
  }

  const timeoutMessage = `No streamed token received within ${input.model.firstTokenTimeoutSeconds} seconds`;
  const remainingMs =
    (input.model.firstTokenTimeoutSeconds * 1000) - (Date.now() - input.attemptStartTime);
  ...
  try {
    ...
    input.abortController.abort(
      error instanceof Error && error.message ? error.message : timeoutMessage,
    );
    try {
      await reader.cancel(timeoutMessage);
    } catch {
    ...
    return {
      ok: false,
      statusCode: 504,
      errorMessage: error instanceof Error ? error.message : timeoutMessage,
    };
  }
...
function createAttemptPath(model: ProxyModel, path?: AttemptPathNode[]): AttemptPathNode[] {
  if (path?.length) {
    return path;
  }
...
function createExecutionFailure(
  ...
  return {
    ...
    errorMessage: failure.errorMessage || responseDetails || 'Unknown upstream provider error',
...
async function attemptModelExecution(
  ...
  if (!input.body.stream) {
    const responsePayload = (await result.response.json()) as Record<string, unknown>;
    return {
      ok: true,
      kind: 'json',
      model: input.model,
  ...
  return {
    ...
    completion: preparedResponse.completion,
    upstreamRequestId,
    fallbackChain: pathToFallbackChain(path),
    attemptStartTime,
  };
...
async function executeRetryTarget(
  ...
  if (isModelBlocked(input.model.id)) {
    ...
    if (input.model.fallbackModelId) {
      const fallbackModel = await getActiveFallbackModel(prisma, input.model.fallbackModelId);
      if (fallbackModel) {
        const fallbackPath = appendAttemptPath(path, fallbackModel);
        if (!fallbackPath) {
          logError('proxy.fallback_cycle_detected', new Error('Fallback cycle detected'), {
            requestId: fallbackContext.requestId,
            requestType: input.requestType,
            fallbackChain: pathToFallbackChain(path),
        ...
        return executeRetryTarget(prisma, {
          ...input,
          model: fallbackModel,
        }, fallbackContext, fallbackPath);
      }
    }

    logError('proxy.model_blocked_no_fallback', new Error('Model is temporarily blocked and has no fallback'), {
      requestId: fallbackContext.requestId,
      modelId: input.model.id,
      modelName: input.model.displayName,
      blockedUntil: blockExpiry ? new Date(blockExpiry).toISOString() : null,
    ...
    return createExecutionFailure(
      ...
      {
        statusCode: 503,
        errorMessage: 'Model is temporarily blocked (sticky fallback) and no fallback model available',
        upstreamRequestId: null,
      },
      path,
  ...
  if (!input.model.fallbackModelId) {
    ...
    for (let retryAttempt = 1; retryAttempt <= input.model.maxRetries; retryAttempt += 1) {
      logError('proxy.retrying_same_model', new Error(lastFailure.errorMessage), {
        requestId: fallbackContext.requestId,
        requestType: input.requestType,
        modelName: input.model.displayName,
        retryAttempt,
    ...
    logError('proxy.all_fallbacks_exhausted', new Error(lastFailure.errorMessage), {
      requestId: fallbackContext.requestId,
      requestType: input.requestType,
      modelName: input.model.displayName,
      totalRetryCount: fallbackContext.totalRetryCount,
  ...
  const fallbackModel = await getActiveFallbackModel(prisma, input.model.fallbackModelId);
  if (!fallbackModel) {
    logError('proxy.fallback_model_unavailable', new Error('Fallback model not found or inactive'), {
      requestId: fallbackContext.requestId,
      fallbackModelId: input.model.fallbackModelId,
    });
    return wrapExhaustedFailure(createExecutionFailure(
      input.model,
      {
        ...
        errorMessage: initialResult.errorMessage || 'Fallback model unavailable',
  ...
  for (let retryAttempt = 1; retryAttempt <= input.model.maxRetries; retryAttempt += 1) {
    ...
    const fallbackResult = await executeRetryTarget(prisma, {
      ...input,
      model: fallbackModel,
    }, fallbackContext, fallbackPath);
...
export async function forwardChatCompletion(
  ...
  if (!executionResult.ok) {
    await recordUsageLog(prisma, executionResult.model, {
      ...
      requestPayload: logging.logPayloads ? (input.body as PrismaTypes.InputJsonValue) : null,
      responsePayload: logging.logPayloads
        ? ({ error: executionResult.errorMessage } as PrismaTypes.InputJsonValue)
        : null,
      errorMessage: executionResult.errorMessage,
      upstreamRequestId: executionResult.upstreamRequestId,
      durationMs: Date.now() - requestStartTime,
      isFallback: fallbackContext.totalRetryCount > 0 || fallbackContext.isStickyFallback || false,
      isStickyFallback: fallbackContext.isStickyFallback || false,
      originalModelId: fallbackContext.totalRetryCount > 0 || fallbackContext.isStickyFallback
  ...
  if (executionResult.kind === 'json') {
    ...
    await recordUsageLog(prisma, executionResult.model, {
      ...
      statusCode: executionResult.statusCode,
      requestPayload: logging.logPayloads ? (input.body as PrismaTypes.InputJsonValue) : null,
      responsePayload: executionResult.responsePayload as PrismaTypes.InputJsonValue,
      upstreamRequestId: executionResult.upstreamRequestId,
      durationMs: Date.now() - requestStartTime,
      isFallback: fallbackContext.totalRetryCount > 0 || fallbackContext.isStickyFallback || false,
      isStickyFallback: fallbackContext.isStickyFallback || false,
    ...
    return Response.json(executionResult.responsePayload, { status: executionResult.statusCode });
  }

  void executionResult.completion
    .then(async ({ usagePayload, assistantText }) => {
      const attemptDurationMs = Date.now() - executionResult.attemptStartTime;
      ...
      await recordUsageLog(prisma, executionResult.model, {
        ...
        requestPayload: logging.logPayloads ? (input.body as PrismaTypes.InputJsonValue) : null,
        responsePayload: logging.logPayloads
          ? ({ usage: usagePayload?.usage || null, assistantText } as PrismaTypes.InputJsonValue)
    ...
    .catch(async (error) => {
      ...
      await recordUsageLog(prisma, executionResult.model, {
        ...
        errorMessage: error instanceof Error ? error.message : 'Failed to inspect stream',
        upstreamRequestId: executionResult.upstreamRequestId,
        durationMs: Date.now() - requestStartTime,
        isFallback: fallbackContext!.totalRetryCount > 0 || fallbackContext!.isStickyFallback || false,
        isStickyFallback: fallbackContext!.isStickyFallback || false,
