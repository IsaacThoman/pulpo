...
Explicit preset choices (such as reasoning and speed) in the regular web/desktop new-chat composer also update the account's per-model defaults. Fresh composers remember those choices even when draft synchronization is disabled or no shared draft is available. Choices in existing chats and temporary composers remain scoped to their drafts; restoring a shared draft or submitted turn does not change account defaults. Redirect presets keep the selected catalog model as the chat identity, with the hidden execution model attached only to the response.
